...
let cubeMesh, frontCameraScreenMesh, backCameraScreenMesh
...
const createCube = () => {
  ...
  const cornerSegments = 1  // 增加分段数，让圆角更平滑，减少视觉上的线条

  // 创建圆角立方体几何体
  const geometry = new RoundedBoxGeometry(cubeSize, cubeSize, cubeSize, cornerSegments, cornerRadius)
...
const createSphereGrid = () => {
  ...
  for (let lat = -90; lat <= 90; lat += 18) {
    ...
    // 关键线条加深
    const isKeyLine = (lat === -90 || lat === -45 || lat === 0 || lat === 45 || lat === 90)
    const opacity = isKeyLine ? 0.3 : 0.15

    const mat = new THREE.LineBasicMaterial({ color: 0xdddddd, transparent: true, opacity: opacity })
  ...
  for (let lon = -90; lon <= 90; lon += 9) {
    ...
    // 关键线条加深
    const isKeyLine = (lon === -90 || lon === -45 || lon === 0 || lon === 45 || lon === 90)
    const opacity = isKeyLine ? 0.3 : 0.15
...
const createCameraModel = () => {
  ...
  const frontScreenMaterial = new THREE.MeshBasicMaterial({
    color: 0xffffff,
    side: THREE.DoubleSide
  })
  ...
  const buttonMaterial = new THREE.MeshPhongMaterial({ color: 0x202020 })
  ...
  const backScreenMaterial = new THREE.MeshBasicMaterial({
    color: 0xffffff,
    side: THREE.DoubleSide
  })
...
const updateCubeTexture = () => {
  if (props.imageUrl && cubeMesh) {
    // 加载原始图片以获取尺寸
    const img = new Image()
    img.crossOrigin = 'anonymous'
    img.onload = () => {
      // 创建一个正方形的 canvas（立方体面是正方形）
      const canvas = document.createElement('canvas')
